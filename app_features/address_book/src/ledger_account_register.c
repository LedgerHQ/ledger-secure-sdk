...
/**
 * @file ledger_account_register.c
 * @brief Register Ledger Account flow (P1=0x11, struct type 0x2f)
 *
 * Flow:
...
{
    UNUSED(context);
    if (!tlv_enforce_u8_value(data, TYPE_REGISTER_LEDGER_ACCOUNT)) {
        PRINTF("[Register Ledger Account] Invalid STRUCTURE_TYPE value\n");
        return false;
    }
    return true;
}
...
static bool handle_struct_version(const tlv_data_t *data, s_ledger_account_ctx *context)
{
    ...
    if (!tlv_enforce_u8_value(data, STRUCT_VERSION)) {
        PRINTF("[Register Ledger Account] Invalid STRUCTURE_VERSION value\n");
        return false;
    }
    return true;
}
...
static void print_payload(const s_ledger_account_ctx *context)
{
    ...
    PRINTF("[Ledger Account] -    Blockchain family:   %s\n",
...
/**
 * @brief Register a new Ledger Account
 *
...
bolos_err_t register_ledger_account(uint8_t *buffer_in, size_t buffer_in_length)
{
    ...
    if (!ledger_account_tlv_parser(&payload, &ctx, &ctx.received_tags)) {
        PRINTF("[Ledger Account] Failed to parse TLV payload\n");
    ...
    // Display confirmation UI (delegated to coin app)
