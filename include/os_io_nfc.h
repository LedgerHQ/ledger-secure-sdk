...
bool nfc_reader_send(const uint8_t      *cmd_data,
                     size_t              cmd_len,
                     nfc_resp_callback_t callback,
                     int                 timeout_ms);

/* Return false if nfc reeder can not be started in current conditions */
bool nfc_reader_start(nfc_evt_callback_t callback);
void nfc_reader_stop(void);
bool nfc_is_reader(void);
