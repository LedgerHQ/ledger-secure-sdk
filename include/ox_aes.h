 ...
 * - EDDSA
 * - ECDH
 * - Schnorr signature with different implementations, especially the one used for Zilliqa
 ...
 *
 * @details Such container should be initialized with cx_ees_init_key to ensure future API
 *          compatibility. Indeed, in next API level, the key store format may changed at all.
 *          Only 16-byte key (AES128) is supported .
 */
struct cx_aes_key_s {
