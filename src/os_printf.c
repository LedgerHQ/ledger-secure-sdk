...
#pragma GCC diagnostic pop
    ...
    --dstlen;  // dstlen includes the terminating '\0'
    size_t len = srclen;
    ...
    if (len > dstlen) {
        ...
        size_t nb_truncated_bytes = find_invalid_trailing_bytes(src, len);

        // Update len with the number of bytes that need to be skipped from the end
