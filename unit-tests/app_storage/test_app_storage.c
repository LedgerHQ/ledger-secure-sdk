...
/* Basic geeter fucntions with initially empty app storage */
static void test_getters_from_empty(void **state __attribute__((unused)))
{
    ...
    /* The properties are fixed in CMakeListList.txt as APP_STORAGE_PROP_SETTINGS | APP_STORAGE_PROP_DATA */
    assert_int_equal(app_storage_get_properties(), APP_STORAGE_PROP_SETTINGS | APP_STORAGE_PROP_DATA);
}
...
{
    /* buf = NULL */
    assert_int_equal(app_storage_pread(NULL, 5, 0), APP_STORAGE_EINVAL);

    /* nbytes = 0 */
    uint8_t buf[20];
    memset(buf, 0xAA, sizeof(buf));
    assert_int_equal(app_storage_pread(buf, 0, 0), APP_STORAGE_EINVAL);

    /* size + offset integer overlfow */
    assert_int_equal(app_storage_pread(buf, UINT32_MAX-4, 5), APP_STORAGE_EINVAL);

    /* Reading 1 byte with 0 offset on empty app storage */
    assert_int_equal(app_storage_pread(buf, 1, 0), APP_STORAGE_EADDRNOTAVAIL);
}
...
{
    /* buf = NULL */
    assert_int_equal(app_storage_pwrite(NULL, 5, 0), APP_STORAGE_EINVAL);

    /* nbytes = 0 */
    uint8_t buf[20];
    memset(buf, 0xAA, sizeof(buf));
    assert_int_equal(app_storage_pwrite(buf, 0, 0), APP_STORAGE_EINVAL);

    /* size + offset integer overlfow */
    assert_int_equal(app_storage_pwrite(buf, UINT32_MAX-6, 7), APP_STORAGE_EINVAL);

    /* Wring outside APP_STORAGE_SIZE */
    assert_int_equal(app_storage_pwrite(buf, APP_STORAGE_SIZE + 1, 0), APP_STORAGE_EOVERFLOW);

    /* Wring outside APP_STORAGE_SIZE */
    assert_int_equal(app_storage_pwrite(buf, 1, APP_STORAGE_SIZE), APP_STORAGE_EOVERFLOW);
}
...
static void test_data_version_from_empty(void **state __attribute__((unused)))
{
    ...
    assert_int_equal(app_storage_pwrite(buf_in, INITIAL_SIZE + ADDITIONALL_SIZE, 0), INITIAL_SIZE + ADDITIONALL_SIZE);
    assert_int_equal(app_storage_get_data_version(), APP_STORAGE_INITIAL_APP_DATA_VERSION);
...
static void test_write_read_from_empty(void **state __attribute__((unused)))
{
    ...
    /* 1, 2, 3 ... */
    assert_int_equal(app_storage_pwrite(buf_in, INITIAL_SIZE, 0), INITIAL_SIZE);
    app_storage_increment_data_version();
    ...
    assert_int_equal(app_storage_pread(buf_out, INITIAL_SIZE, 0), INITIAL_SIZE);
    assert_memory_equal(buf_in, buf_out, INITIAL_SIZE);

    /* Trying to read 1 byte more */
    assert_int_equal(app_storage_pread(buf_out, INITIAL_SIZE + 1, 0), APP_STORAGE_EADDRNOTAVAIL);

    /* Trying to read correct size but with offset = 1 */
    assert_int_equal(app_storage_pread(buf_out, INITIAL_SIZE, 1), APP_STORAGE_EADDRNOTAVAIL);

    /* Reading with offset = 1 */
    memset(buf_out, 0, sizeof(buf_out));
    assert_int_equal(app_storage_pread(buf_out, INITIAL_SIZE - 1, 1), INITIAL_SIZE - 1);
    assert_memory_equal(&buf_in[1], buf_out, INITIAL_SIZE - 1);


    /* Next write */
    /* 1, 2, 3 ... 21, 22, 23 */
    assert_int_equal(app_storage_pwrite(&buf_in[INITIAL_SIZE], ADDITIONALL_SIZE, INITIAL_SIZE), ADDITIONALL_SIZE);
    app_storage_increment_data_version();
    ...
    /* Reading all */
    memset(buf_out, 0, sizeof(buf_out));
    assert_int_equal(app_storage_pread(buf_out, INITIAL_SIZE + ADDITIONALL_SIZE, 0), INITIAL_SIZE + ADDITIONALL_SIZE);
    assert_memory_equal(buf_in, &buf_out, INITIAL_SIZE + ADDITIONALL_SIZE);

    /* Reading at offset = INITIAL_SIZE */
    memset(buf_out, 0, sizeof(buf_out));
    assert_int_equal(app_storage_pread(&buf_out[INITIAL_SIZE], ADDITIONALL_SIZE, INITIAL_SIZE), ADDITIONALL_SIZE);
...
static void test_write_big_reset_from_empty(void **state __attribute__((unused)))
{
    ...
    for (; i < APP_STORAGE_SIZE; i++) {
        buf = (uint8_t)i;
        assert_int_equal(app_storage_pwrite(&buf, 1, i), 1);
        app_storage_increment_data_version();
        assert_int_equal(app_storage_get_data_version(), APP_STORAGE_INITIAL_APP_DATA_VERSION + i + 1);
        assert_int_equal(app_storage_get_size(), i + 1);
    }

    /* Cannot write more */
    buf = (uint8_t)i;
    assert_int_equal(app_storage_pwrite(&buf, 1, i), APP_STORAGE_EOVERFLOW);
    ...
    buf = 0;
    for (; i < APP_STORAGE_SIZE; i++) {
        assert_int_equal(app_storage_pread(&buf, 1, i), 1);
    ...
    buf = 0;
    assert_int_equal(app_storage_pwrite(&buf, 1, APP_STORAGE_SIZE - 1), 1);
...
static void test_write_read_from_prepared(void **state __attribute__((unused)))
{
    ...
    assert_int_equal(app_storage_pwrite(&buf_in[INITIAL_SIZE], ADDITIONALL_SIZE2, INITIAL_SIZE), ADDITIONALL_SIZE2);
    app_storage_increment_data_version();

    assert_int_equal(app_storage_get_size(), INITIAL_SIZE + ADDITIONALL_SIZE2);

    memset(buf_out, 0, INITIAL_SIZE + ADDITIONALL_SIZE2);
    assert_int_equal(app_storage_pread(buf_out, INITIAL_SIZE + ADDITIONALL_SIZE2, 0), INITIAL_SIZE + ADDITIONALL_SIZE2);
    ...
    assert_int_equal(app_storage_pwrite(&buf_in3[INITIAL_SIZE + ADDITIONALL_SIZE3 - 1], 1, INITIAL_SIZE + ADDITIONALL_SIZE3 - 1), 1);
    app_storage_increment_data_version();
    assert_int_equal(app_storage_get_size(), INITIAL_SIZE + ADDITIONALL_SIZE3);

    uint8_t buf_out3[INITIAL_SIZE + ADDITIONALL_SIZE3] = {0};

    assert_int_equal(app_storage_pread(buf_out3, INITIAL_SIZE + ADDITIONALL_SIZE3, 0), INITIAL_SIZE + ADDITIONALL_SIZE3);
...
static void test_app_style_from_empty(void **state __attribute__((unused)))
{
    ...
    for (uint32_t i = 0; i < SLOT_NUMBER; i++) {
        slot_t slot = {0xA454B0C5, {(i + 1) * 1, (i + 1) * 2, (i + 1) * 3}};
...
{
    /* Checking data version */
    assert_int_equal(app_storage_get_data_version(), APP_STORAGE_INITIAL_APP_DATA_VERSION + 1 + SLOT_NUMBER);
    /* Checking data size */
    ...
    for (uint32_t i = 0; i < SLOT_NUMBER; i++) {
        const slot_t slot_c = {0xA454B0C5, {(i + 1) * 1, (i + 1) * 2, (i + 1) * 3}};
        slot_t slot = {0};
...
int main(int argc, char **argv)
{
    const struct CMUnitTest tests[] = {
        ...
        cmocka_unit_test_setup_teardown(test_write_big_reset_from_empty, setup_from_empty, teardown),
        cmocka_unit_test_setup_teardown(test_write_read_from_prepared, setup_from_prepared, teardown),
        cmocka_unit_test_setup_teardown(test_app_style_from_empty, setup_from_empty, teardown),
        cmocka_unit_test_setup_teardown(test_app_style_from_prepared, setup_from_prepared_app_style, teardown),
    };
