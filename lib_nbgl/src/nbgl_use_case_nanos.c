...
static void drawStep(nbgl_stepPosition_t pos, const nbgl_icon_details_t *icon, const char *txt, const char *subTxt) {
  ...
  if (icon == NULL) {
    context.stepCtx = nbgl_stepDrawText(pos, buttonCallback, NULL, txt, subTxt, BOLD_TEXT1_INFO, false);
  }
  else {
  ...
  if (context.type == REVIEW_USE_CASE) {
    if (context.review.onNav != NULL) {
      ...
      if (context.currentPage == 0) {// title page
        ...
        context.step.subText = context.review.tagValueList.pairs[context.currentPage-1].value;
      }
    }
  }
    ...
    if (context.currentPage < (context.nbPages-1)) {
      // try to get content for this page/step
      if  ((context.settings.onNav == NULL) || (context.settings.onNav(context.currentPage, &context.step) == false)) {
        return;
      }
 ...
 * @param actionCallback callback called when one of the navigations page is selected (double button)
 */
