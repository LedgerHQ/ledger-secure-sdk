                                 ...
                                 bool           wrapping)
{
    ...
    while ((textLen) && (maxNbLines > 0)) {
        ...
        // if \n, reset width
        if (unicode == '\n') {
            // FIX: don't decrease nbLines if we are at the beginning of a new line
            if (width != 0) {
                maxNbLines--;
            }
            width = 0;
                                       ...
                                       uint16_t      *width)
{
    ...
    // TODO: This does not take in account unicode texts!
    *width = 0;
    *len   = 0;
    while (textLen) {
                                    ...
                                    bool           wrapping)
{
    ...
    while (textLen) {
        ...
        // if \n, increment the number of lines
            ...
            // FIX: don't increase nbLines if we are at the beginning of a new line
            if (width != 0) {
                nbLines++;
#ifdef BUILD_SCREENSHOTS
                if (last_nb_lines < nbLines) {
