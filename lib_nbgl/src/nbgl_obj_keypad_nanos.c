 ...
 *  STATIC VARIABLES
 **********************/
static const nbgl_icon_details_t *digits_icons[] = {
  &C_digit_0,
  &C_digit_1,
  &C_digit_2,
  &C_digit_3,
...
void nbgl_keypadCallback(nbgl_obj_t *obj, nbgl_buttonEvent_t buttonEvent) {
  ...
  else if ((buttonEvent == BUTTON_LEFT_PRESSED) || (buttonEvent == BUTTON_LEFT_CONTINUOUS_PRESSED)) {
  ...
  else if ((buttonEvent == BUTTON_RIGHT_PRESSED) || (buttonEvent == BUTTON_RIGHT_CONTINUOUS_PRESSED)) {
    switch(keypad->selectedKey) {
      case 10: // '9'
        if (keypad->enableValidate) {
          keypad->selectedKey = 11;
