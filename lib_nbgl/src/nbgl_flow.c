...
// returns the flow context from the contexts[] array matching with the given step handler, or NULL if not found
...
static void drawStep(FlowContext_t* ctx, nbgl_stepPosition_t pos, bool modal, const nbgl_icon_details_t *icon, const char *txt, const char *subTxt) {
  ...
  if (icon == NULL) {
    ctx->stepCtx = nbgl_stepDrawText(pos, actionCallback, NULL, txt, subTxt, REGULAR_INFO, modal);
  }
  else {
    info.icon = icon;
    info.text1 = txt;
...
// function called on key action of the current step, if not an internal navigation in a multi-pages text step
static void actionCallback(nbgl_step_t stepCtx, nbgl_buttonEvent_t event) {
