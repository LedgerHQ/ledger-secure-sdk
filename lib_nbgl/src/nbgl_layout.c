    ...
    while (i < layout->container->nbChildren) {
        if (layout->container->children[i]->type == CONTAINER) {
            ...
            if (container->children[0]->type == SPINNER) {
                ...
                spinner->position++;
                if (spinner->position == SPINNER_FIXED) {
                    spinner->position = 0;
                }
                             ...
                             uint8_t        position)
{
    ...
    // if position his different, redraw
    ...
    // if text his different, redraw
    if (!strcmp(textArea->text, text)) {
        textArea->text = text;
    ...
    if (subText != NULL) {
        ...
        // if text his different, redraw
        if (!strcmp(subTextArea->text, text)) {
            subTextArea->text = subText;
