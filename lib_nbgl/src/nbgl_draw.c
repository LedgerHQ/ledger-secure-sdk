                             ...
                             color_t            fontColor)
{
    ...
    while (textLen > 0) {
        ...
        if (!char_byte_cnt || encoding == 1) {
            // To be able to handle transparenc, ramBuffer must be filled with background color
        ...
        // if (!strcmp(original_text, "Continue setting up using the Ledger Live app")) {
            ...
            // Draw that character in a RAM buffer (X & Y are rotated 90° on Stax/Flex)
