                             ...
                             color_t            fontColor)
{
    ...
    while (textLen > 0 || next_unicode) {
        ...
        if (!char_byte_cnt || encoding == 1) {
            ...
            if (unicode >= 0x000E00 && unicode < 0x000E80 && area->y0 == 124) {
                fprintf(stdout,
            ...
            if (!over_previous) {
                ...
                if (redraw_buf_area) {
                    ...
                    if (previous_unicode == 0x000E49 && unicode == 0x000E07
                        && next_unicode == 0x000E2B && area->y0 == 124 && !ultrasaved) {
                        ultrasaved = 1;
        ...
        // Compute next x with current char_width, to be ready to display next one
                ...
                if (unicode != 0x00E31 && (x + char_width + char_x_min) > next_x) {
                    // if (strstr(original_text, "ถอนการติดตั้งแอปทั้งหมด")) {
                    if (!strcmp(original_text, "เปิด")) {
                        fprintf(stdout,
                                "Increasing(2) next_x from %d to %d (char_width=%d, char_x_min=%d, "
                                "unicode='%c'(0x0%X))\n",
                                next_x,
                                x + char_width + char_x_min,
                                char_width,
                                char_x_min,
                                unicode,
                                unicode);
                    }
                    next_x = x + char_width + char_x_min;
            ...
            if (!strcmp(original_text, "เปิด")
                && (unicode == 0x000E14 || unicode == 0x000E40 || unicode == 0x000E1B
                    || unicode == 0x000E34)
                && !saved) {
                if (unicode == 0x000E14) {
                    saved = 1;
