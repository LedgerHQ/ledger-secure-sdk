...
#define SAVE_PACKED_PICTURE
#endif  // BUILD_SCREENSHOTS

#ifdef SAVE_PACKED_PICTURE  // TMP Wip stuff
                                ...
                                uint32_t unicode)
{
    ...
    if (bpp == NBGL_BPP_4) {
        sprintf(filename, "toto_4bpp_0x%X.png", unicode);
        ...
        sprintf(filename, "toto_1bpp_0x%X.png", unicode);
        for (uint16_t y = 0; y < height; y++) {
                                     ...
                                     uint32_t unicode)
{
    ...
    if (bpp == NBGL_BPP_4) {
        sprintf(filename, "roro_4bpp_0x%X.png", unicode);
        for (uint16_t y = 0; y < height; y++) {
            for (uint16_t x = 0; x < width; x += 2) {
                uint8_t double_pixel        = *packed_buffer++;
                buffer[(y * width) + x + 0] = double_pixel >> 4;
        ...
        sprintf(filename, "roro_1bpp_0x%X.png", unicode);
        uint8_t pixels = *packed_buffer++;
        uint8_t msk    = 0x80;
        for (uint16_t y = 0; y < height; y++) {
...
#ifdef SAVE_PACKED_PICTURE
    if (!strcmp(text, THE_STRING) && !ultrasaved) {
        if (current == 0x0E2D) {
            ultrasaved = 1;
        }
                             ...
                             color_t            fontColor)
{
    ...
    while (textLen > 0 || next_char.unicode) {
        ...
        if (!current_char.byte_cnt || current_char.encoding == 1) {
            ...
            // nbgl_frontDrawImage(&current_area, current_char.buffer, NO_TRANSFORMATION,
            // fontColor);
        ...
        if (!strcmp(original_text, THE_STRING) && !saved) {
            if (current_char.unicode == 0x000E2D) {
                saved = 1;
            }
            fprintf(stdout,
                    ...
                    buf_y_min,
                    current_char.nb_skipped_bytes);
            fprintf(stdout, "ramBuffer: width=%d, height=%d\n", buf_area.width, buf_area.height);
            fflush(stdout);
#ifdef SAVE_PACKED_PICTURE
            save_packed_picture(
                ramBuffer, buf_area.width, buf_area.height, current_area.bpp, current_char.unicode);
#endif  // SAVE_PACKED_PICTURE
        }
#endif  // BUILD_SCREENSHOTS
#endif  // 0
