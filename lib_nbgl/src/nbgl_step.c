...
typedef struct TextContext_s {
  ...
  nbgl_stepButtonCallback_t onActionCallback;   ///< function called when key actions done on this step (other than internal navigation)
  char tmpString[TMP_STRING_MAX_LEN];           ///< temporary string used for title when text + multi-pages subText
...
typedef struct MenuListContext_s {
  nbgl_stepMenuListCallback_t selectedCallback; ///< function to call when a menu list item is selected
  nbgl_layoutMenuList_t list;                   ///< structure to store the number of menu items and the way to select them
} MenuListContext_t;
...
// returns the step context from the contexts[] array matching with the given layout handler, or NULL if not found
...
static const char *getTextPageAt(StepContext_t *ctx, uint8_t textPage) {
  ...
  while (page < textPage) {
    if (page < (ctx->textContext.nbPages-1)) {
      uint16_t len;
      nbgl_getTextMaxLenInNbLines(BAGL_FONT_OPEN_SANS_REGULAR_11px_1bpp, currentChar, AVAILABLE_WIDTH, NB_MAX_LINES, &len, true);
...
static const char *getSubTextPageAt(StepContext_t *ctx, uint8_t textPage) {
  ...
  while (page < textPage) {
    if (page < (ctx->textContext.nbPages-1)) {
      uint16_t len;
      nbgl_getTextMaxLenInNbLines(BAGL_FONT_OPEN_SANS_REGULAR_11px_1bpp, currentChar, AVAILABLE_WIDTH, NB_MAX_LINES-1, &len, true);
      currentChar = currentChar+len;
    }
    page++;
  }
...
static void displayTextPage(StepContext_t *ctx, uint8_t textPage) {
  ...
  if (ctx->textContext.currentPage < (ctx->textContext.nbPages-1)) {
    uint16_t len;
    uint8_t nbLines = (ctx->textContext.subTxtStart == NULL) ? NB_MAX_LINES : (NB_MAX_LINES-1);
    nbgl_getTextMaxLenInNbLines(BAGL_FONT_OPEN_SANS_REGULAR_11px_1bpp, txt, AVAILABLE_WIDTH, nbLines, &len, true);
  ...
  navInfo.indication = getNavigationInfo(ctx->textContext.pos,ctx->textContext.nbPages,ctx->textContext.currentPage);
  ...
  if (ctx->textContext.subTxtStart == NULL) {
    ...
    if (ctx->textContext.nbPages == 1) {
      ...
      SPRINTF(ctx->textContext.tmpString, "%s (%d/%d)", ctx->textContext.txtStart, ctx->textContext.currentPage+1, ctx->textContext.nbPages);
...
static void actionCallback(nbgl_layout_t *layout, nbgl_buttonEvent_t event) {
  ...
  if (event == BUTTON_LEFT_PRESSED) {
    ...
    else if ((ctx->textContext.pos == LAST_STEP) || (ctx->textContext.pos == NEITHER_FIRST_NOR_LAST_STEP)) {
    ...
    else if ((ctx->textContext.pos == FIRST_STEP) || (ctx->textContext.pos == NEITHER_FIRST_NOR_LAST_STEP)) {
      ctx->textContext.onActionCallback((nbgl_step_t)ctx, event);
    }
  }
...
static void displayMenuList(StepContext_t *ctx) {
  nbgl_layoutDescription_t layoutDescription = {
    .modal = ctx->modal,
    .onActionCallback = menuListActionCallback
  };
...
/**
 * @brief draws a text type step, that can be multi-pages, depending of the length of text and subText.
 *        The navigation arrows are displayed depending of the given position of this step in a flow, and depending
 *        of the page in case of multi-pages
 *
...
nbgl_step_t nbgl_stepDrawText(nbgl_stepPosition_t pos,
  ...
  if (subText == NULL) {
    ctx->textContext.nbPages = nbgl_getTextNbPagesInWidth(BAGL_FONT_OPEN_SANS_REGULAR_11px_1bpp, text, NB_MAX_LINES, AVAILABLE_WIDTH);
  }
  else {
    // NB_MAX_LINES-1 because first line is for main text
    ctx->textContext.nbPages = nbgl_getTextNbPagesInWidth(BAGL_FONT_OPEN_SANS_REGULAR_11px_1bpp, subText, NB_MAX_LINES-1, AVAILABLE_WIDTH);
  }
  LOG_DEBUG(STEP_LOGGER,"nbgl_stepDrawText: ctx = %p, nbPages = %d, pos = 0x%X\n", ctx, ctx->textContext.nbPages, pos);
...
nbgl_step_t nbgl_stepDrawCenteredInfo(nbgl_stepPosition_t pos,
  ...
  nbgl_layoutDescription_t layoutDescription = {
    .modal = false,
    .onActionCallback = (nbgl_layoutButtonCallback_t)actionCallback
  };
  nbgl_layoutNavigation_t navInfo = {
    .direction = HORIZONTAL_NAV,
  };
...
/**
 * @brief draw a step page with a menu list and navigation arrows to parse it. This step must be alone
 *
 * @param onActionCallback common callback for all actions on this page
 * @param ticker ticker configuration, set to NULL to disable it
