...
static void draw_progressBar(nbgl_progress_bar_t* obj, nbgl_obj_t *prevObj, bool computePosition) {
  ...
  if (obj->withBorder) {
    nbgl_drawRoundedBorderedRect((nbgl_area_t*)obj,
                                RADIUS_3_PIXELS, stroke, obj->obj.area.backgroundColor, obj->foregroundColor);
  } else {
    nbgl_drawRoundedRect((nbgl_area_t*)obj,
                              RADIUS_3_PIXELS, obj->obj.area.backgroundColor);
  }
  // draw level
...
static void draw_textArea(nbgl_text_area_t* obj, nbgl_obj_t *prevObj, bool computePosition) {
  ...
  for (line=0;line<nbLines;line++) {
    ...
    nbgl_getTextMaxLenAndWidth(fontId,text,obj->obj.area.width,&lineLen,&lineWidth,obj->wrapping);
    ...
    LOG_DEBUG(OBJ_LOGGER,"draw_textArea(), %s line %d, lineLen %d lineWidth = %d, obj.area.height = %d, textHeight = %d, nbMaxLines = %d, wrapping = %d\n",text+3,  line,lineLen, lineWidth,obj->obj.area.height, textHeight, obj->nbMaxLines, obj->wrapping);
...
static void draw_textEntry(nbgl_text_entry_t* obj, nbgl_obj_t *prevObj, bool computePosition) {
  ...
  for (int i=0;i<NB_MAX_LETTERS;i++) {
    char digit;
    rectArea.x0 = obj->obj.area.x0+4+(i*10);
    rectArea.y0 = obj->obj.area.y0;
    rectArea.width = 8;
...
void extendRefreshArea(nbgl_obj_t *obj) {
  ...
  if (obj->area.x0 < refreshArea.x0) {
    refreshArea.x0 = MAX(0,obj->area.x0);
  }
  ...
  if (obj->area.y0 < refreshArea.y0) {
    refreshArea.y0 = MAX(0,obj->area.y0);
  }
