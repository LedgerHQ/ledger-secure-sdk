...
typedef struct {
    ...
    // Set once a SEPROXYHAL_TAG_USB_EP_XFER_OUT_64_ZEROPADDED event is received: proves the
...
#endif  // TARGET_APEX_P
#if defined(TARGET_APEX_M)
        usbd_ledger_data.product = USBD_LEDGER_PRODUCT_APEX_M;
#endif  // TARGET_APEX_M
...
{
    if (ep_num >= sizeof(usbd_ledger_data.usb_ep_out_64_zeropadded_seen)
                      / sizeof(usbd_ledger_data.usb_ep_out_64_zeropadded_seen[0])) {
        return false;
    }
    ...
    if (seph_buffer[1] == SEPROXYHAL_TAG_USB_EVENT) {
        ...
        switch (seph_buffer[5]) {
            ...
            case SEPROXYHAL_TAG_USB_EP_XFER_OUT_64_ZEROPADDED:
                if ((epnum < IO_USB_MAX_ENDPOINTS) && (length <= LEDGER_USBD_DEFAULT_EPOUT_SIZE)) {
                    ...
                    if (length < LEDGER_USBD_DEFAULT_EPOUT_SIZE) {
                        ...
                        size_t padding_size = LEDGER_USBD_DEFAULT_EPOUT_SIZE - length;
                        memset(&seph_buffer[7 + length], 0, padding_size);
                    }

                    // Send packet as a regular data out
                    USBD_LEDGER_rx_evt_data_out(
                        epnum, &seph_buffer[7], LEDGER_USBD_DEFAULT_EPOUT_SIZE);
                    status = USBD_LEDGER_data_ready(apdu_buffer, apdu_buffer_max_length);
                }
                else {
                    goto error;
