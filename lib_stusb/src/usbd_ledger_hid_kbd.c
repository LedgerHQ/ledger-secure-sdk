...
uint8_t USBD_LEDGER_HID_KBD_init(USBD_HandleTypeDef *pdev, void *cookie)
{
    ...
    USBD_LL_PrepareReceive(pdev, LEDGER_HID_KBD_EPOUT_ADDR, NULL, LEDGER_HID_KBD_EPOUT_SIZE);
    ...
    if (pdev->dev_state == USBD_STATE_CONFIGURED) {
        if (handle->state == LEDGER_HID_KBD_STATE_IDLE) {
            ret = USBD_LL_Transmit(
                pdev, LEDGER_HID_KBD_EPIN_ADDR, packet, LEDGER_HID_KBD_EPIN_SIZE, timeout_ms);
        }
        else {
            ret = USBD_BUSY;
        }
