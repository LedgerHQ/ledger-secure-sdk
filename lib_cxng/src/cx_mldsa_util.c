                                 ...
                                 size_t         in2len)
{
    ...
    if (total <= sizeof(buf)) {
        ...
        error = cx_shake256_hash(buf, total, out, outlen);
        explicit_bzero(buf, total);
    }
    else {
        cx_sha3_t ctx = {0};
        error         = cx_shake256_init_no_throw(&ctx, outlen * 8U);
        if (error != CX_OK) {
            return error;
        }
        error = cx_hash_no_throw((cx_hash_t *) &ctx, 0, in1, in1len, NULL, 0);
        if (error != CX_OK) {
            return error;
        }
        error = cx_hash_no_throw((cx_hash_t *) &ctx, CX_LAST, in2, in2len, out, outlen);
    }
    return error;
}
                                   ...
                                   size_t         in3len)
{
    ...
    error = cx_hash_no_throw((cx_hash_t *) &ctx, 0, in2, in2len, NULL, 0);
    if (error != CX_OK) {
        return error;
    }
    return cx_hash_no_throw((cx_hash_t *) &ctx, CX_LAST, in3, in3len, out, outlen);
}
