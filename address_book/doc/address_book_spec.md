...
| 0x0B | SCOPE                  | Context string for the identifier (printable ASCII, max 32 chars)                     |
| 0x0C | PREVIOUS_CONTACT_NAME  | Previous contact name, for display and HMAC verification (Edit Contact Name only)     |
| 0x0D | IDENTIFIER             | Blockchain identifier — address (Ethereum, Solana) or public key (Bitcoin); max 80 B  |
| 0x0E | PREVIOUS_IDENTIFIER    | Previous identifier, for HMAC verification (Edit Identifier only)                     |
| 0x0F | PREVIOUS_SCOPE         | Previous scope, for display and HMAC verification (Edit Scope only)                   |
| 0x10 | GROUP_HANDLE           | Device-generated opaque token: `gid(32) + HMAC-SHA256(K_group, gid)(32)`; see §4.3    |
...
| 0x54 | HMAC_PROOF             | 32-byte HMAC-SHA256 name-binding proof: covers `gid(32) + name` (§4.3)                |
| 0x55 | HMAC_REST              | 32-byte HMAC-SHA256 over `(gid(32), scope, identifier, family[, chain_id])` (§4.3)    |
...
| 0x12        | `TYPE_EDIT_CONTACT_NAME`       | Edit Contact Name        |
| 0x13        | `TYPE_REGISTER_LEDGER_ACCOUNT` | Register Ledger Account  |
| 0x14        | `TYPE_EDIT_LEDGER_ACCOUNT`     | Edit Ledger Account      |
| 0x15        | `TYPE_EDIT_IDENTIFIER`         | Edit Identifier          |
| 0x16        | `TYPE_EDIT_SCOPE`              | Edit Scope               |
...
Registers a `(name, scope, identifier)` tuple on the device. The `IDENTIFIER` field is blockchain-agnostic:
...
| SCOPE             | 0x0B  | Yes       | 32 B     | Context string (e.g. "Ethereum", "Bitcoin legacy", "Solana USDC")     |
| IDENTIFIER        | 0x0D  | Yes       | 80 B     | Blockchain identifier (address or pubkey, chain-dependent)            |
...
sequenceDiagram
    ...
    D->>W: 0x11 | group_handle(64) | hmac_proof(32) | hmac_rest(32)  [9000]
...
- **Sub-command:** P1 = `0x02`
- **Structure type:** `0x12` (`TYPE_EDIT_CONTACT_NAME`)
- **Guard:** `HAVE_ADDRESS_BOOK`
...
| Tag Name              | Value | Mandatory | Max size | Description                                                           |
|-----------------------|-------|-----------|----------|-----------------------------------------------------------------------|
| STRUCT_TYPE           | 0x01  | Yes       | 1 B      | 0x12 (`TYPE_EDIT_CONTACT_NAME`)                                       |
| STRUCT_VERSION        | 0x02  | Yes       | 1 B      | 0x01                                                                  |
...
| GROUP_HANDLE          | 0x10  | Yes       | 64 B     | Opaque token from Register Identity response                          |
| DERIVATION_PATH       | 0x21  | Yes       | 41 B     | BIP32 derivation path (same as at registration)                       |
| HMAC_PROOF            | 0x54  | Yes       | 32 B     | `HMAC_PROOF` returned by the original Register Identity               |
...
- `type` = `0x12` (`TYPE_EDIT_CONTACT_NAME`)
- `hmac_proof` = new HMAC-SHA256 over: `gid(32) | name_len(1) | new_contact_name`

### 5.3 Edit Identifier

- **Sub-command:** P1 = `0x03`
- **Structure type:** `0x15` (`TYPE_EDIT_IDENTIFIER`)
- **Guard:** `HAVE_ADDRESS_BOOK`
...
| Tag Name            | Value | Mandatory | Max size | Description                                                              |
|---------------------|-------|-----------|----------|--------------------------------------------------------------------------|
| STRUCT_TYPE         | 0x01  | Yes       | 1 B      | 0x15 (`TYPE_EDIT_IDENTIFIER`)                                            |
| STRUCT_VERSION      | 0x02  | Yes       | 1 B      | 0x01                                                                     |
| CONTACT_NAME        | 0x0A  | Yes       | 32 B     | Contact name (unchanged from registration)                               |
| SCOPE               | 0x0B  | Yes       | 32 B     | Context string (unchanged from registration)                             |
| IDENTIFIER          | 0x0D  | Yes       | 80 B     | New blockchain identifier (replacing the previous one)                   |
| PREVIOUS_IDENTIFIER | 0x0E  | Yes       | 80 B     | Previous identifier (to verify the HMAC from the prior registration)     |
| GROUP_HANDLE        | 0x10  | Yes       | 64 B     | Opaque token from Register Identity response                             |
| DERIVATION_PATH     | 0x21  | Yes       | 41 B     | BIP32 derivation path (same as at registration)                          |
| CHAIN_ID            | 0x23  | Cond.     | 8 B      | Chain ID (mandatory for Ethereum)                                        |
| BLOCKCHAIN_FAMILY   | 0x51  | Yes       | 1 B      | Blockchain family                                                        |
| HMAC_PROOF          | 0x54  | Yes       | 32 B     | `HMAC_PROOF` from Register Identity (verifies the contact name)          |
| HMAC_REST           | 0x55  | Yes       | 32 B     | `HMAC_REST` from Register Identity (verifies the previous identifier)    |
...
| Tag Name               | Value | Mandatory | Max size | Description                                                           |
|------------------------|-------|-----------|----------|-----------------------------------------------------------------------|
| STRUCT_TYPE            | 0x01  | Yes       | 1 B      | 0x16 (`TYPE_EDIT_SCOPE`)                                              |
| STRUCT_VERSION         | 0x02  | Yes       | 1 B      | 0x01                                                                  |
| CONTACT_NAME           | 0x0A  | Yes       | 32 B     | Contact name (unchanged from registration)                            |
| SCOPE                  | 0x0B  | Yes       | 32 B     | New scope (max 32 printable ASCII chars)                              |
| IDENTIFIER             | 0x0D  | Yes       | 80 B     | Blockchain identifier (unchanged from registration)                   |
| PREVIOUS_SCOPE         | 0x0F  | Yes       | 32 B     | Previous scope (must match value used at registration)                |
| GROUP_HANDLE           | 0x10  | Yes       | 64 B     | Opaque token from Register Identity response                          |
| DERIVATION_PATH        | 0x21  | Yes       | 41 B     | BIP32 derivation path (same as at registration)                       |
| CHAIN_ID               | 0x23  | Cond.     | 8 B      | Chain ID (mandatory for Ethereum)                                     |
| BLOCKCHAIN_FAMILY      | 0x51  | Yes       | 1 B      | Blockchain family                                                     |
| HMAC_PROOF             | 0x54  | Yes       | 32 B     | `HMAC_PROOF` from Register Identity, proves the contact name          |
| HMAC_REST              | 0x55  | Yes       | 32 B     | `HMAC_REST` from Register Identity, proves the scope                  |
...
- **Sub-command:** P1 = `0x12`
- **Structure type:** `0x14` (`TYPE_EDIT_LEDGER_ACCOUNT`)
- **Guard:** `HAVE_ADDRESS_BOOK` && `HAVE_ADDRESS_BOOK_LEDGER_ACCOUNT`
...
| Tag Name              | Value | Mandatory | Max size | Description                                                           |
|-----------------------|-------|-----------|----------|-----------------------------------------------------------------------|
| STRUCT_TYPE           | 0x01  | Yes       | 1 B      | 0x14 (`TYPE_EDIT_LEDGER_ACCOUNT`)                                     |
| STRUCT_VERSION        | 0x02  | Yes       | 1 B      | 0x01                                                                  |
...
| HMAC_PROOF            | 0x54  | Yes       | 32 B     | HMAC Proof of Registration returned by Register Ledger Account        |
| DERIVATION_PATH       | 0x21  | Yes       | 41 B     | BIP32 derivation path (same as at registration)                       |
| CHAIN_ID              | 0x23  | Cond.     | 8 B      | Chain ID (mandatory for Ethereum)                                     |
| BLOCKCHAIN_FAMILY     | 0x51  | Yes       | 1 B      | Blockchain family                                                     |
...
- `type` = `0x14` (`TYPE_EDIT_LEDGER_ACCOUNT`)
...
- **Sub-command:** P1 = `0x20`
- **Structure type:** `0x20` (`TYPE_PROVIDE_CONTACT`)
- **Guard:** `HAVE_ADDRESS_BOOK`
...
| Tag Name          | Value | Mandatory | Max size | Description                                                            |
|-------------------|-------|-----------|----------|------------------------------------------------------------------------|
| STRUCT_TYPE       | 0x01  | Yes       | 1 B      | 0x20 (`TYPE_PROVIDE_CONTACT`)                                          |
| STRUCT_VERSION    | 0x02  | Yes       | 1 B      | 0x01                                                                   |
| CONTACT_NAME      | 0x0A  | Yes       | 32 B     | Contact name (must match value used at registration)                   |
| SCOPE             | 0x0B  | Yes       | 32 B     | Contact scope (must match value used at registration)                  |
| IDENTIFIER        | 0x0D  | Yes       | 80 B     | Blockchain identifier (unchanged from registration)                    |
| GROUP_HANDLE      | 0x10  | Yes       | 64 B     | Opaque token from Register Identity response                           |
| DERIVATION_PATH   | 0x21  | Yes       | 41 B     | BIP32 derivation path (same as at registration)                        |
| CHAIN_ID          | 0x23  | Cond.     | 8 B      | Chain ID (mandatory for Ethereum)                                      |
| BLOCKCHAIN_FAMILY | 0x51  | Yes       | 1 B      | Blockchain family                                                      |
| HMAC_PROOF        | 0x54  | Yes       | 32 B     | `HMAC_PROOF` from Register Identity — proves contact name              |
| HMAC_REST         | 0x55  | Yes       | 32 B     | `HMAC_REST` from Register Identity — proves scope + identifier         |
