...
#define LEDGER_ACCOUNT_TAGS(X)                                                   \
    ...
    X(0x0B, TAG_ACCOUNT_NAME, handle_account_name, ENFORCE_UNIQUE_TAG)           \
    X(0x75, TAG_DERIVATION_PATH, handle_derivation_path, ENFORCE_UNIQUE_TAG)     \
    X(0x76, TAG_DERIVATION_SCHEME, handle_derivation_scheme, ENFORCE_UNIQUE_TAG) \
...
static bool handle_derivation_scheme(const tlv_data_t *data, s_ledger_account_ctx *context)
{
    ...
    if (value < DERIVATION_SCHEME_BIP44 || value > DERIVATION_SCHEME_GENERIC_DESCRIPTOR) {
...
static void print_payload(const s_ledger_account_ctx *context)
{
    ...
    PRINTF("[Ledger Account] -    Derivation path length: %d\n",
           context->ledger_account->derivation_path_length);
    PRINTF("[Ledger Account] -    Derivation scheme: %d\n",
           context->ledger_account->derivation_scheme);
...
/**
 * @brief Parse the TLV payload containing the Ledger Account.
 *
 * @param[in] payload buffer_t with the complete TLV payload
 * @return whether the TLV payload was handled successfully or not
 */
bool handle_ledger_account_tlv_payload(const buffer_t *payload)
{
    bool                 ret = false;
    ...
    ret = ledger_account_tlv_parser(payload, &ctx, &ctx.received_tags);
    if (ret) {
