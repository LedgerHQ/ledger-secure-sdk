 ...
 *
 * HMAC messages:
 *  - Identity:       name_len(1) | name | scope_len(1) | scope | id_len(1) | identifier
 *  - Ledger Account: name_len(1) | name | family(1) [| chain_id_be(8) for FAMILY_ETHEREUM]
 *
 ...
 * (SHA256(salt || privkey.d)) and initialises the HMAC context.
 * Callers must explicit_bzero() both @p private_key and @p hmac_key on exit.
 *
 * @param[in]  bip32_path  BIP32 path
 * @param[in]  salt        KDF salt (without null terminator)
 * @param[in]  salt_len    Salt length
 * @param[out] private_key Derived private key (caller zeroes on exit)
 * @param[out] hmac_key    Derived HMAC key (caller zeroes on exit)
 ...
 *
 * @param[in] type       Message type (TYPE_REGISTER_IDENTITY or TYPE_REGISTER_LEDGER_ACCOUNT)
...
/**
 * @brief Compute an HMAC-SHA256 Proof of Registration for an Identity contact.
 *
 * HMAC key: SHA256("AddressBook-Identity" || privkey.d)
 * HMAC message: name_len(1) | name | scope_len(1) | scope | id_len(1) | identifier
 *
 * @param[in]  bip32_path     BIP32 path used at registration
 * @param[in]  name           Contact name (null-terminated)
 * @param[in]  scope          Contact scope (null-terminated, may be empty string)
 * @param[in]  identifier     Raw identifier bytes (address or public key)
 * @param[in]  identifier_len Length of identifier in bytes
                                              ...
                                              uint8_t             hmac_out[CX_SHA256_SIZE])
{
    ...
    if (!init_hmac_ctx(bip32_path, salt, sizeof(salt) - 1, &private_key, hmac_key, &hmac_ctx)) {
        goto end;
    }
    // name_len(1) | name
    ...
    CX_CHECK(cx_hmac_no_throw(
        (cx_hmac_t *) &hmac_ctx, CX_LAST, identifier, identifier_len, hmac_out, CX_SHA256_SIZE));
    success = true;
...
/**
 * @brief Verify an HMAC Proof of Registration for an Identity contact.
 *
 * Re-derives and compares the HMAC using constant-time comparison.
 *
 * @param[in] bip32_path     BIP32 path used at registration
 * @param[in] name           Contact name (null-terminated)
 * @param[in] scope          Contact scope (null-terminated, may be empty string)
                                             ...
                                             const uint8_t       hmac_expected[CX_SHA256_SIZE])
{
    ...
    if (!address_book_compute_hmac_proof_identity(
            bip32_path, name, scope, identifier, identifier_len, hmac_computed)) {
        goto end;
    }
    if (memcmp(hmac_computed, hmac_expected, CX_SHA256_SIZE) != 0) {
 ...
 * HMAC message: name_len(1) | name | family(1) [| chain_id_be(8)]
 *
 * chain_id_be(8) is included only when family == FAMILY_ETHEREUM.
 *
 * @param[in]  bip32_path  BIP32 path used at registration
