...
static bool handle_derivation_path(const tlv_data_t *data, s_ext_addr_ctx *context)
{
    ...
    if (!bip32_path_read(path_buffer.ptr,
                         path_buffer.size,
                         (uint32_t *) context->external_address->derivation_path,
                         nb_elements)) {
        PRINTF("DERIVATION_PATH: failed to parse BIP32 path\n");
        return false;
    }
    // Store the path length (number of elements)
    context->external_address->derivation_path_length = path_buffer.size / 4;
    return true;
}
...
static void print_payload(const s_ext_addr_ctx *context)
{
    ...
    if (bip32_path_format(context->external_address->derivation_path,
                          context->external_address->derivation_path_length,
                          out,
                          sizeof(out))) {
