/* Absolution dispatcher for lib_alloc (stateful, byte-coded command stream). */
...
static void exec_realloc(size_t index, size_t len)
{
    ...
    if (len == 0) {
        /* mem_realloc(ctx, old, 0) behaves like mem_free(ctx, old) and always
         * returns NULL: `old` is already freed, so drop it from the tracking
         * table. Treating this the same as a plain allocation failure below
         * would leave a stale entry pointing at freed memory, and a later
         * command on the same index would double-free it and corrupt the
