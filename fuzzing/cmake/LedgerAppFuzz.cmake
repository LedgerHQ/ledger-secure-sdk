...
# Absolution is auto-fetched from GitHub releases; ABSOLUTION_DIR / sibling
...
# Ensures `<path>` exists by copying from `<template_path>` (or writing the
# literal `${content}` payload when no template is given). Intended for the
# files the SDK self-fuzz tree and CFL builds expect Absolution to refine
# during the first build of a fresh checkout.
...
# App file validation: ensures the contract files the framework needs exist.
# Missing `fuzz_globals.zon` / `scenario_layout.h` are bootstrapped from the
# SDK template so fresh clones and CI containers configure cleanly without
# committing layout-dependent artifacts. `mocks.h` is app-owned and stays a
# hard requirement.
