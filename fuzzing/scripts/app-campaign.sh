...
# Each fuzzer's `invariants/<target>.zon` is a machine-local snapshot of the
# Absolution global model. It encodes concrete `.whole_values` blobs whose
# sizes depend on the build's global memory layout (sanitizer choice, SDK
# revision, toolchain). A snapshot from one machine/config is **not portable**:
# Absolution rejects mismatched blobs with `WholeValuesBlobMismatch`.
#
# BOOTSTRAP_INVARIANT controls when to reset the file to `.{}` (empty model),
# which forces Absolution to re-discover the layout from scratch and lets the
# `sync_invariant` step below regenerate a model that matches the current
# build:
#   1     — always reset before configure (recommended for CI / CFL).
#   0     — never reset (use the committed/checked-out file as-is).
#   auto  — reset only when --clean was passed. (default)
