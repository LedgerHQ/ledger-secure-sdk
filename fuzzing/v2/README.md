...
This is the v2 framework. The v1 framework remains at `fuzzing/` (top
level) for backward compatibility; v2 internals live under `fuzzing/v2/`
with `ledger_fuzz_*` CMake targets. The v2 public surface (`fuzzing/cmake/`,
`fuzzing/scripts/`, `fuzzing/sdk-fuzz/`, `fuzzing/template/`, `fuzzing/docs/`)
is unchanged. Apps only add their own `fuzzing/` subtree; nothing is copied
out of the SDK.
...
Paths are relative to `${BOLOS_SDK}/fuzzing/`. The v2 public surface stays at
the top level; v2 internals live under `v2/` so the restored v1 tree at the
top level keeps working unchanged.

| Path                 | Purpose                                                                                   |
|----------------------|-------------------------------------------------------------------------------------------|
| `cmake/`             | `LedgerAppFuzz.cmake` — the CMake module apps include (v2 public surface)                 |
...
| `mock/gen_mock.py`   | Generator for weak syscall stubs (shared with v1)                                         |
| `v2/include/`        | Framework headers + optional TLV grammar-aware mutator                                    |
| `v2/mock/cx/`        | Strong crypto, big-number, and EC point mocks                                             |
| `v2/mock/nbgl/`      | NBGL runtime and use-case mocks                                                           |
| `v2/mock/os/`        | OS, PIC, exception, libc, NVM, and I/O runtime shims                                      |
| `v2/mock/_generated/`| Generated weak syscall stubs                                                              |
| `v2/libs/`           | Per-library CMake modules aggregated into the `ledger_fuzz_secure_sdk` INTERFACE target   |
| `v2/macros/`         | Build macro extraction and add / exclude lists                                            |
| `v2/invariants/`     | SDK-level zero-symbol policy (applied to every app)                                       |
| `v2/sanitizers/`     | UBSan / ASan runtime config and ignorelists                                               |
