...
./local_run.sh --build=1 --fuzzer=build/fuzz_lists --j=4 --run-fuzzer=1 --BOLOS_SDK=/path/to/sdk
```
...
```bash
./local_run.sh --build=1 --fuzzer=build/fuzz_lists --compute-coverage=1 --BOLOS_SDK=/path/to/sdk
```
