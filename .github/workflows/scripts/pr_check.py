...
def get_pull_request(repo: Repository, pull_number: int) -> PullRequest:
    ...
    try:
        ...
        logger.debug(f"Pull Title: {pull.title}")
        logger.debug(f"Pull Body:\n{pull.body}")
        return pull
    except Exception as e:
        logger.error(f"Failed to retrieve pull request: {e}")
        sys.exit(1)
...
def get_target_branches(pull: PullRequest) -> List[str]:
    ...
    if not pull.body:
        ...
        sys.exit(0)
    print(f"Pull Body:\n{pull.body}")
    x = re.search(r"^TARGET_API_LEVEL:[\s]*(.*)", pull.body)
    if not x:
        logger.warning("No target branch specified for cherry-pick. Exiting.")
        sys.exit(0)
    target_branches = [grp.strip() for grp in x.groups()]
    if len(target_branches) == 0:
        logger.warning("No target branch specified for cherry-pick. Exiting.")
        sys.exit(0)
    logger.debug(f"Target Branches: {target_branches}")
...
def get_all_branches(repo: Repository) -> List[str]:
    ...
    try:
        ...
        logger.error(f"Failed to retrieve branches: {e}")
        sys.exit(1)
...
def validate_target_branches(target_branches: List[str], branches: List[str]) -> None:
    ...
    for target_br in target_branches:
        if target_br not in branches:
            logger.error(f"Target branch '{target_br}' doesn't exist! Aborting.")
            sys.exit(1)
...
def get_commits_to_cherry_pick(pull: PullRequest) -> List[str]:
    ...
    if len(commits) == 0:
        logger.error(f"No commits found. Exiting.")
        sys.exit(1)
    logger.info(f"Found commits: {commits}")
    return commits
...
def cherry_pick_commits(repo: Repository, commits: List[str], auto_branch: str) -> None:
    ...
    logger.info(f"Target branch '{my_br.name}' sha: {br_sha}")
    ...
    for com in commits:
        ...
        # Create a new commit with the same message and tree, but with the current branch's latest commit as the parent
        try:
            ...
            logger.error(f"Failed to create commit!")
            sys.exit(1)

        # Update the branch SHA to the new commit's SHA
        br_sha = new_commit.sha
        logger.debug(f"Cherry-picked commit {commit.sha} to {new_commit.sha}")

    # Update the reference of the branch to the new commit
    try:
        ref = repo.get_git_ref(f"heads/{auto_branch}")
    except:
        logger.error(f"Failed to get git reference!")
        sys.exit(1)
    ref.edit(sha=br_sha)
    logger.info(f"Updated branch '{auto_branch}' to new commit {br_sha}")
...
def create_pull_request_if_needed(repo: Repository, auto_branch: str, target_br: str) -> None:
    ...
    if not any(pr.title == pr_auto_title for pr in prs):
        # Create a new pull request with the auto_update branch as the head and the target branch as the base
        try:
            repo.create_pull(
                title=pr_auto_title,
        ...
        logger.info(f"Created PR for branch '{auto_branch}'")
...
def main():
    ...
    try:
        ...
        logger.error(f"Repo NameGithub connection error: {err}")
        sys.exit(1)

    # Retrieve repo object
    repo = github.get_repo(f"{GITHUB_ORG_NAME}/{args.repo}")
    logger.debug(f"Repo Name: {repo.name}")
