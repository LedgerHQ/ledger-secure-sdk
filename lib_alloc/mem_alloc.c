 ...
 *      DEFINES
 *********************/
// nb bytes of a pointer
#define ADDR_WIDTH            sizeof(size_t)
// sizeof of header for an allocated chunk
#define ALLOCATED_HEADER_SIZE (2 * ADDR_WIDTH)
// size of heap header
#define HEAP_HEADER_SIZE      sizeof(heap_t)
...
typedef struct header_s {
    ...
    struct header_s *phys_prev;  ///< physical previous chunk

    struct header_s *fprev;  ///< previous free chunk in the same segment
    struct header_s *fnext;  ///< next free chunk in the same segment
} header_t;
...
typedef struct {
    ...
    header_t *free_segments[NUM_SEG_LISTS];
} heap_t;
...
static inline void list_remove(header_t **first_free, header_t *header)
{
    // if was the first, set a new first
    if (*first_free == header) {
        *first_free = (*first_free)->fnext;
        // if the current first was not alone
        if (*first_free) {
            (*first_free)->fprev = NULL;
        }
        return;
    }
    // link previous to following, if existing previous
    if (header->fprev != NULL) {
        header->fprev->fnext = header->fnext;
    }
    // link following to previous, if existing following
    if (header->fnext != NULL) {
        header->fnext->fprev = header->fprev;
    }
}
...
{
    header_t **first = &heap->free_segments[seglist_index(heap, header->size)];
...
mem_ctx_t mem_init(void *heap_start, size_t heap_size)
{
    ...
    memset(heap->free_segments, 0, heap->nb_segs * sizeof(header_t *));
...
void *mem_alloc(mem_ctx_t ctx, size_t nb_bytes)
{
    ...
    nb_bytes += ADDR_WIDTH - 1;
    nb_bytes &= ~(ADDR_WIDTH - 1);
    if (nb_bytes < ALLOCATED_HEADER_SIZE) {
        nb_bytes = ALLOCATED_HEADER_SIZE;
    }
    nb_bytes += ALLOCATED_HEADER_SIZE;  // for header (size +  prev_chunk)
    ...
    while (seg < heap->nb_segs) {
        header_t *header = heap->free_segments[seg];
        ...
        if (header != NULL) {
            ...
            return block + ALLOCATED_HEADER_SIZE;
        }
...
{
    heap_t   *heap   = (heap_t *) ctx;
    uint8_t  *block  = ((uint8_t *) ptr) - ALLOCATED_HEADER_SIZE;
    ...
    // try previous chunk
    if (header->phys_prev != NULL) {
        header = coalesce(heap, header, header->phys_prev);
    }
...
void mem_parse(mem_ctx_t ctx, mem_parse_callback_t callback, void *data)
{
    ...
    while (header != NULL) {
        ...
        if (callback(data, block + ADDR_WIDTH, header->allocated, header->size)) {
            return;
        }
        if (&block[header->size] < (uint8_t *) heap->end) {
